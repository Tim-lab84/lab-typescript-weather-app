...
export function displayLocation(locationDetails: Location): void {
  ...
  const countryNameElm = document.getElementById("country") as HTMLElement;
  countryNameElm.innerText = `(${locationDetails.country})`;
}
...
export function displayWeatherData(obj: WeatherResponse): void {
  ...
  windspeedElm.innerText = `Windspeed: ${windspeed} ${windspeedUnits}`;
...
export function updateBackground(weatherCode: number, isDay: number): void {
  const firstLetter = weatherCode.toString().charAt(0);
  switch (firstLetter) {
    case "0":
    case "1":
      if (isDay === 0) {
